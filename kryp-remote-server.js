...
const server=http.createServer((req,res)=>{
  ...
  if(u.pathname==='/health') return httpJson(res,200,{ok:true,cursor,clients:clients.size,events:events.length});
  if(u.pathname==='/api/events' && req.method==='GET'){
    const role=u.searchParams.get('role')||''; const token=req.headers['x-kryp-token']||'';
    if(!authorized(role,token)) return httpJson(res,401,{error:'unauthorized'});
  ...
  if(u.pathname==='/api/events' && req.method==='POST'){
    readJson(req,data=>{
      const role=data && data.role; const token=req.headers['x-kryp-token']||data && data.token;
      if(!authorized(role,token)) return httpJson(res,401,{error:'unauthorized'});
  ...
  if((req.method==='GET' || req.method==='HEAD') && (u.pathname==='/' || u.pathname==='/index.html')){
    try{
      ...
      console.error('index.html serve failed:',e.message);
      return res.writeHead(500).end('Frontend unavailable');
    }
  }
...
server.on('upgrade',(req,socket)=>{
  ...
  const role=u.searchParams.get('role')||''; const token=u.searchParams.get('token')||'';
  if(!authorized(role,token)){socket.write('HTTP/1.1 401 Unauthorized\r\n\r\n');socket.destroy();return;}
